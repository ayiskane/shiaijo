...
const TournamentManager = memo(function TournamentManager({
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="bg-[#142130] border-white/5">
        <CardHeader className="p-3 cursor-pointer" onClick={() => setShowEditTournament(!showEditTournament)}>
          <div className="flex items-center justify-between">
            <CardTitle className="text-white text-sm flex items-center gap-2">
              <Settings className="w-4 h-4 text-orange-400" />
              Edit Tournament
            </CardTitle>
            <ChevronDown className={`w-4 h-4 text-slate-500 transition-transform ${showEditTournament ? '' : '-rotate-90'}`} />
          </div>
        </CardHeader>
        {showEditTournament && (
          <CardContent className="p-3 pt-0 space-y-3">
            {/* Timer Options */}
            <div className="bg-[#1e3a5f]/20 rounded-lg p-2.5 border border-white/5">
              <span className="text-xs text-[#8fb3d1] block mb-2">Timer Options</span>
                  ...
                  return (
                    <button
                      ...
                      className={`px-2.5 py-1 rounded text-xs font-medium transition-all ${
                        isSelected ? 'bg-blue-600 text-white' : 'bg-slate-700/50 text-slate-400 hover:bg-slate-700'
                      }`}
                    >
            ...
            <div className="flex gap-2 flex-wrap">
              <Button 
                onClick={refreshTournamentParticipants} 
                variant="outline" 
                size="sm"
                className="h-8 border-[#2a4a6f] bg-[#1a2d42] hover:bg-[#1e3a5f]"
              >
              ...
              <Button 
                ...
                className="h-8 border-red-700/60 text-red-400 bg-red-900/20 hover:bg-red-800/40"
              >
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button
            ...
            size="sm"
            onClick={() => {
              const allGroupIds = tournament.groupOrder || []
              const allCollapsed = allGroupIds.every(id => collapsedGroups.includes(id))
              setCollapsedGroups(allCollapsed ? [] : allGroupIds)
            }}
            className="h-8 border-[#1e3a5f] text-[#8fb3d1]"
          >
              ...
              >
                <CardHeader className={`p-3 ${isCollapsed ? 'pb-3' : 'pb-2'}`}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`px-1.5 h-5 rounded flex items-center justify-center text-[10px] font-bold flex-shrink-0 ${
                      isShared ? 'bg-emerald-500 text-white' : isCourtA ? 'bg-amber-500 text-black' : 'bg-blue-500 text-white'
                    }`}>
                      {isShared ? 'A+B' : isCourtA ? 'A' : 'B'}
                    </span>
                    <span className="text-white font-medium text-sm flex-1 truncate">{group?.name || groupId}</span>
                    {isNonBogu && <span className="text-[8px] px-1 py-0.5 bg-orange-900/40 text-orange-300 rounded flex-shrink-0">Hantei</span>}
                    <button
                      onClick={(e) => { e.stopPropagation(); setEditingGroupId(isEditing ? null : groupId) }}
                      className={`p-1 rounded transition flex-shrink-0 ${isEditing ? 'text-orange-400 bg-orange-500/20' : 'text-[#6b8fad] hover:text-white'}`}
                    >
                      <Edit2 className="w-3.5 h-3.5" />
                    </button>
                    <span className="text-[10px] text-[#6b8fad] flex-shrink-0">{completedCount}/{groupMatches.length}</span>
                  </div>
...
const SortableMatchItem = memo(function SortableMatchItem({
  ...
  // NEXT is for matches that are selected but NOT the current match (i.e., queued for later)
  const isNextInQueue = isSelectedByThisCourt && !isCurrentMatch && !isLiveOnThisCourt
  ...
  return (
    <div
      ...
      className={`relative w-full p-2 rounded-lg mb-1 text-xs cursor-pointer select-none transition-colors ${
        ...
        : isNextInQueue ? 'bg-amber-900/40 border-2 border-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.3)]'
        : isSelectedByOtherCourt ? 'bg-slate-800/30 border border-slate-600 opacity-50'
        : 'bg-slate-800/50 hover:bg-slate-800'
      }`}
    >
      <div className="flex items-center">
        {canDrag && !isNextInQueue && !isLiveOnThisCourt && !isActiveHantei && !isCurrentNotStarted && (
          <span 
            ref={setActivatorNodeRef}
            {...attributes}
            {...listeners}
        ...
        {isLiveOnThisCourt && (
          <span className="text-[8px] px-1.5 py-0.5 rounded font-bold mr-2 bg-emerald-500 text-white">
            LIVE
          </span>
        )}
        {isActiveHantei && (
          <span className="text-[8px] px-1.5 py-0.5 rounded font-bold mr-2 bg-orange-500 text-white">
            NOW
          </span>
        )}
        {isCurrentNotStarted && (
        ...
        {isNextInQueue && <span className="text-[8px] px-1.5 py-0.5 rounded bg-amber-500 text-black font-bold mr-2">NEXT</span>}
              ...
              {showGroupQueue && (
                <div className="px-3 pb-2 space-y-1">
                  ...
                  {groupOrder.map((groupId, gIdx) => {
                    ...
                    return (
                      <div key={groupId} className="flex items-center text-xs py-1.5 gap-1.5">
                        <span className="text-slate-300 flex-1 truncate">{groupInfo?.name} <span className="text-slate-500">({groupMatchCount})</span></span>
                        <button
                          onClick={() => moveGroupInQueue(groupId, 'up')}
                          disabled={gIdx === 0}
                          className="w-5 h-5 rounded bg-slate-800 text-slate-500 disabled:opacity-30 flex items-center justify-center"
                        ...
                        {(() => {
                          const groupCourt = pendingMatches.find(m => m.groupId === groupId)?.court || 'A'
                          const isOnA = !isShared && groupCourt === 'A'
                          const isOnB = !isShared && groupCourt === 'B'
                          return (
                            <div className="flex rounded overflow-hidden border border-slate-700">
                              <button
